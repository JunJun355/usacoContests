...
def run(n, m, edges):
    ...
    while tosearch:
        ...
        for new in edges[curr]:
            ...
            searched.add(new)
            tosearch.append(new)
            lowestn = min(lowestn, new)
    # print(lowestn, highest1)
    curr_lowest = abs(lowestn - highest1) ** 2
    if abs(lowestn - highest1) > 1:
        avg = (lowestn + highest1) // 2
        curr_lowest = min(abs(lowestn - avg) ** 2 + abs(highest1 - avg) ** 2, curr_lowest)
    for check in edges.keys():
        # print(check, curr_lowest)
